...
export function createGame(player1Id: number, player2Id: number, maxGames: number, timeStarted: string):
Promise<number> {
...
export function endGame(gameId: string, score1: number, score2: number
): Promise<void> {
	return new Promise((resolve, reject) => {
		db.run(
			`UPDATE games
			...
			function (err: Error | null) {
				if (err) {
					console.error('❌ Erro ao terminar jogo:', err.message);
					reject(err);
				} 
				else
					resolve();
...
export function getPlayersFromGame(gameId: number): Promise<GamePlayers> {
	return new Promise((resolve, reject) => {
		db.get(
			...
			[gameId],
			(err: Error | null, row: { player1_id: number; player2_id: number } | undefined) => {
				if (err) {
					console.error('❌ Erro ao obter jogadores:', err.message);
					reject(err);
				} 
				else if (!row)
					reject(new Error(`⚠️ Jogo com ID ${gameId} não encontrado.`));
				else
					resolve({player1Id: row.player1_id, player2Id: row.player2_id,});
			}
		);
	});
}
