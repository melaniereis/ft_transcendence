...
function runAsync(query: string, params: any[] = []): Promise<void> {
    return new Promise((resolve, reject) => {
        db.run(query, params, function (err: Error | null) {
            if (err) {
                console.error(`❌ Failed to run query: ${query}`, err.message);
                reject(err);
            } 
			else
                resolve();
...
function getAsync<T = any>(query: string, params: any[] = []): Promise<T | undefined> {
    return new Promise((resolve, reject) => {
        db.get(query, params, (err: Error | null, row: unknown) => {
            if (err) {
                console.error(`❌ Failed to get record: ${query}`, err.message);
                reject(err);
            } 
			else
                resolve(row as T);
...
function allAsync<T = any>(query: string, params: any[] = []): Promise<T[]> {
    return new Promise((resolve, reject) => {
        db.all(query, params, (err: Error | null, rows: unknown[] | undefined) => {
            if (err) {
                console.error(`❌ Failed to get records: ${query}`, err.message);
                reject(err);
            } 
			else
                resolve(rows as T[]);
...
export async function createUser(name: string, username: string, team: string, password: string): Promise<void> {
    ...
    if (!teamKey)
        throw new Error('⚠️ Invalid team name');

    const existingUser = await getUserByUsername(username);
    if (existingUser)
        throw new Error('⚠️ Username already exists');
