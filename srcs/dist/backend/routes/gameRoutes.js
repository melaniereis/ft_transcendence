...
export async function gameRoutes(fastify) {
    fastify.post('/games', async (req, reply) => {
        const { player1_id, player2_id, max_games, time_started } = req.body;
        try {
    ...
    fastify.put('/games/:gameId/end', async (req, reply) => {
        ...
        const { score_player1, score_player2 } = req.body;
        try {
...
export async function syncUserStatsToTeam(id) {
    const user = await getUserById(id);
    if (!user)
        throw new Error(`User ${id} not found`);
    const teamTable = validTeams[user.team.toUpperCase()];
    if (!teamTable)
    ...
    console.log('teamTable:', teamTable, '| user.id:', user.id, '| user.username:', user.username, '| stats.matches_won:', stats.matches_won, '| stats.tournaments_won:', stats.tournaments_won, '| stats.matches_lost:', stats.matches_lost, '| winRate:', winRate);
    await updateTeamMember(teamTable, user.id, user.username, stats.matches_won, stats.tournaments_won, stats.matches_lost, winRate);
}
