...
export async function startTournament(container: HTMLElement, tournament: any, selectedPlayers: any[], authToken?: string) {
  ...
  // async function fetchGame(playerA: number, playerB: number) {
  //   if (!token) throw new Error('Authentication token required');
  //   const res = await fetch('/games', {
  //     method: 'POST',
  //     headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  //     body: JSON.stringify({ player1_id: playerA, player2_id: playerB, max_games: maxGames, time_started: new Date().toISOString() })
  //   });
  ...
  renderTournamentBracket(canvas, ctx, players, winners, async () => {
    ...
    // const game1 = await fetchGame(player1.id, player2.id);
    state.player1 = null;
    state.player2 = null;
    state.ball = null;
                  ...
                  await endGame(
                    ...
                    async (winner2Id) => {
                      ...
                      renderTournamentBracket(canvas, ctx, players, winners, async () => {
                        ...
                        // const finalGame = await fetchGame(finalLeftId, finalRightId);
                        state.player1 = null;
                        state.player2 = null;
                        state.ball = null;
