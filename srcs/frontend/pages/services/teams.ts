...
export async function renderTeamsPage(container: HTMLElement) {
	...
	if (!token) {
		...
		if (!document.getElementById('teams-dreamy-styles')) {
			...
			style.textContent = `
		.teams-bg { position:fixed;inset:0;width:100vw;height:100vh;overflow:hidden;z-index:0;pointer-events:none;background:transparent; }
		...
		.teams-bg, .teams-gradient, .cloud { pointer-events: none !important; }
		`;
