...
import {GRID_COLORS, svgMedalGold, svgTrendIcon, svgBarChartIcon,
svgFlameIcon, svgChartIcon, svgClockIcon} from './constants.js';
import {averageScore, bestPerformance, consistencyScore,
clutchFactor, dominanceRating, efficiencyScore, gamesThisWeek,
longestWinStreak, mostActiveTime } from './metrics.js';

function getTranslationNamespace(lang: keyof typeof translations) {
	return translations[lang].stats;
}

function getLang(): keyof typeof translations {
	return (['en', 'es', 'pt'].includes(localStorage.getItem('preferredLanguage') || '')
		? localStorage.getItem('preferredLanguage')
		: 'en') as keyof typeof translations;
}
const lang = getLang();
const t = getTranslationNamespace(lang);

export function statsOverview(stats: Stats, history: Match[]): string {
	const wr = (stats.win_rate * 100).toFixed(1);
	const kd = stats.points_conceded
		? (stats.points_scored / stats.points_conceded).toFixed(2)
		: String(stats.points_scored);
	const avgP = stats.matches_played
		? (stats.points_scored / stats.matches_played).toFixed(1)
		: '0';
	const avgC = stats.matches_played
		? (stats.points_conceded / stats.matches_played).toFixed(1)
		: '0';
	...
	return `
		...
		<div class="stats-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:20px;margin-bottom:30px">
			...
			<h4 style="margin:0 0 15px 0;color:${GRID_COLORS.primary};display:flex;align-items:center;gap:8px">
				<span>${svgBarChartIcon()}</span> ${t.scoringStatistics}
			</h4>
			<div>
				${[
				...
				<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;${i < arr.length - 1 ? 'border-bottom:1px solid rgba(0,174,239,0.2)' : ''}">
					<span style="color:${GRID_COLORS.muted}">${row[0]}</span>${row[1]}
				</div>
				`).join('')}
			</div>
			</div>
			...
			<div style="background:linear-gradient(135deg, ${GRID_COLORS.cool} 0%, ${GRID_COLORS.bg} 100%);padding:20px;border-radius:12px;">
			<h4 style="margin:0 0 15px 0;color:${GRID_COLORS.primary};display:flex;align-items:center;gap:8px">
				<span>${svgFlameIcon()}</span> ${t.performanceMetrics}
			</h4>
			<div>
				${[
...
export function statsTrends(stats: Stats): string {
	...
	return `
		...
		${[
			{ label: t.winRate, value: `${(stats.win_rate * 100).toFixed(1)}%`, color: GRID_COLORS.success, icon: svgMedalGold(), period: 'current' },
			{ label: t.avgScore, value: (stats.matches_played ? (stats.points_scored / stats.matches_played).toFixed(1) : '0'), color: GRID_COLORS.cool, icon: svgBarChartIcon(), period: 'per match' },
			{ label: t.gamesPerWeek, value: String(gamesThisWeek([])), color: GRID_COLORS.cool, icon: svgChartIcon(), period: 'this week' },
		].map(t => `
