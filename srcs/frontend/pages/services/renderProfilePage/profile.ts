...
import { layout, statsOverview, historyList } from './templates.js';
...
export async function renderProfilePage(container: HTMLElement, onBadgeUpdate?: () => void) {
    ...
    try {
        ...
        histEl.innerHTML = state.history.length
        ...
        // ✅ Immediately render friends list (no delay)
        rerenderFriends();

        // ✅ Set up general events
        ...
        container.addEventListener('click', (e) => {
            ...
            if (tEl.id === 'friends-prev' || tEl.id === 'friends-next') {
                ...
                // ✅ Then re-render friends list and attach event listeners again
                setTimeout(() => {
                    rerenderFriends();
                }, 350);
            }
